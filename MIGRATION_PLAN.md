...
**Part 2 — `useTiming71` 통합 ✅ 완료**
- `useTiming71.ts`가 GriiipClient 콜백 → `engine.apply*` → `engine.snapshot()`을 사용하도록 교체. 로컬 build*/포맷터/15개 ref 전부 제거(엔진이 소유). 복원·REST 결과·no-service 로직은 보존(`engine.setRestoredCars`/`setMessages`/`getParticipants` 추가).
- 🟠 `flush`를 매 이벤트 → **250ms 코얼레싱 배치**(`scheduleFlush`)로 변경 → 이벤트 버스트당 snapshot 1회.
- `app/types/race.ts`를 `@wec/core` 타입 재export로 전환(기존 `@/app/types/race` import 무변경).
- 엔진에 `setMessages`/`getParticipants` 추가, Vitest 22개로 확장.
- **검증:** `npm run test:core` 22개 통과 + 웹 `next build` `✓ Compiled successfully`(타입 검증 포함).
- **미검증:** 라이브 WEC 세션 런타임(가용 세션 없음) — 엔진 로직은 단위테스트로 커버, 통합부는 빌드/타입으로 커버.
- **롤백:** 코어 import만 되돌리면 됨.

> 참고: 원격 코드의 REST 정규화(`griiipResults.ts`)는 아직 별도 경로 → 후속 단계에서 동일 엔진/포맷터로 수렴시킬 것(현재는 `buildCarsFromResults` → `engine.setRestoredCars`로 연결).

### Phase 1 — Supabase 셋업 + `raceStore` 교체 (다시보기 경로부터)
**목표:** 저장 계층을 Blob/로컬파일 → Supabase로. 프론트 API 표면(`/api/races/*`)은 유지.

**Part 1 — 코드 스캐폴딩 🟡 완료 (라이브 Supabase 검증 대기)**
- `@supabase/supabase-js` 추가.
- `supabase/schema.sql` — §3 DDL(races/snapshots/live_state + RLS) 작성. `supabase/README.md`에 셋업 가이드.
- `app/lib/supabaseAdmin.ts` — service_role 서버 전용 클라이언트 팩토리(미설정 시 `null`).
- `app/lib/raceStore.ts` — **3단 백엔드 자동 선택**(`SUPABASE_*` → Blob → 로컬). `getIndex`/`getRace`/`upsertIndex`/`upsertRace` 시그니처 유지 → `useReplay`/`page.tsx` 무변경. RaceMeta↔races / RaceSnapshot↔snapshots 매핑 추가.
- `appendSnapshot(meta, snapshot)` 신설 — Supabase에선 **단일 행 upsert**(whole-blob RMW 제거, 문제 B 완화). `app/api/races/snapshot/route.ts`가 직접 blob `put` 대신 이를 호출(HTTP 계약 동일).
- `scripts/seed-supabase.ts` + `npm run seed:supabase` — `app/data/races/*.json` 멱등 임포트 + 과거 레이스 `status='finished'`.
- **검증:** `next build` ✓ Compiled successfully(타입체크 포함) + core Vitest 22개 통과. `SUPABASE_*` 미설정 시 기존 Blob/로컬 경로 그대로(비파괴).
- **미검증:** 실제 Supabase 프로젝트 대상 read/write·다시보기 런타임(프로젝트·키 발급은 사용자 작업).

**Part 2 — 라이브 셋업/검증 ⏳ (사용자 작업)**
- Supabase 프로젝트 생성, `supabase/schema.sql` 실행, `.env.local`에 `SUPABASE_URL`+`SUPABASE_SERVICE_ROLE_KEY`.
- `npm run seed:supabase` → 다시보기 탭이 Supabase 데이터로 정상 동작 확인.
- **완료 기준:** 다시보기 탭이 Supabase 데이터로 정상 동작.
- **롤백:** `SUPABASE_*` env 해제 → 자동으로 Blob/로컬 폴백(인터페이스 동일).
...
- [x] Phase 0 Part 2: `useTiming71` → `@wec/core` 교체 + flush 250ms 배치 + `app/types/race.ts` 재export — Vitest 22개 + 웹 빌드 검증 완료 (라이브 런타임 미검증)
- [~] Phase 1 Part 1(코드): `@supabase/supabase-js`, schema.sql, supabaseAdmin, `raceStore` 3단 백엔드+`appendSnapshot`, snapshot 라우트 위임, seed 스크립트 — build+테스트 검증 완료 / Part 2(라이브 Supabase 생성·seed·다시보기 검증)는 사용자 작업 대기
